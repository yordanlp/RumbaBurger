...
void form_ofertas::updateIngredientes(int id)
{
    ...
    foreach (auto i, ingredientsModel) {
        ...
        QTableWidgetItem *amount = new QTableWidgetItem( utiles::truncS(i.amount, 2)  + unit );
        ...
        ui->tw_ingredientes->setItem(row, 2, inversion);
        row++;
    }
}
...
void form_ofertas::on_pb_addOferta_clicked()
{
    ...
    if( dishName == "" ){
        QMessageBox::information(this, "Información", "Escriba el nombre del plato", QMessageBox::Ok);
        return;
    }
...
void form_ofertas::on_pb_delIngrediente_clicked()
{
    ...
    if( res == QMessageBox::Ok ){
        auto newdish = ingredientsService.deleteIngredient( ingredientsModel.at(row) ).data;
        updateIngredientes(newdish.id);
        updatePrecioProduccion(newdish.id);
        updatePrecioSugerido(newdish.id);
        updateOfertas(ui->le_search->text(), rowSelected);
...
void form_ofertas::on_pb_guardar_clicked()
{
    ...
    auto dish = dishModel.at( rowSelected );
    dishService.updateDish(DishDto(dish.id, dishName, descripcion, precio)).data;
    updateOfertas(ui->le_search->text(),rowSelected);
    updateAll();
}
